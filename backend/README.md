# DCBS Backend API (Laravel 11 + MySQL + Sanctum/Passport)
...
- Laravel 11
- MySQL
- Laravel Sanctum (token auth, primary)
- Laravel Passport (fallback when Sanctum is unavailable)
...
- Token creation and protected routes (Sanctum primary, Passport fallback)
...
- Middleware: role middleware + configurable auth guard
- Consistent JSON response format

### 5) Security & Production Practices
- Configurable auth middleware on financial routes (`API_AUTH_GUARD`)
...
5. **If Sanctum is unavailable, switch to Passport**
```bash
...
Then set in `.env`:
```env
API_AUTH_PACKAGE=passport
API_AUTH_GUARD=api
```

And ensure `config/auth.php` has Passport for the `api` guard:
```php
'guards' => [
  'web' => [
    'driver' => 'session',
...
7. **Link storage for file serving (optional)**
```bash
php artisan storage:link
```

8. **Start queue worker (required for statement processing)**
```bash
